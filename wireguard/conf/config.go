...
func (t HandshakeTime) String() string {
	...
	timestamp := strings.Join(s, UnitSeparator)
...
func (conf *Config) IPC() (string, error) {
	buf := &bytes.Buffer{}
	...
	for _, peer := range conf.Peers {
		...
		resolved, err := resolveIpPort(peer.Endpoint.String())
		if err != nil {
			return "", err
		}
...
func resolveIpPort(hostPort string) (string, error) {
	...
	if len(resolved) == 0 {
		return "", fmt.Errorf("cannot find host %s", host)
	}

	return fmt.Sprintf("%s:%s", resolved[0], port), nil
}
